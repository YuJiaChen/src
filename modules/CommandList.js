...
Commands.list = {
    ...
    addmybot: function(gameServer,split) {
        var add = parseInt(split[1]);
        if (isNaN(add)) {
            add = 1; // Adds 1 bot if user doesnt specify a number
        }
    ...
    addgreedybot: function(gameServer,split) {
        var add = parseInt(split[1]);
        if (isNaN(add)) {
            add = 1; // Adds 1 bot if user doesnt specify a number
        }
    ...
    playerlist: function(gameServer,split) {
        ...
        console.log(" ID         | IP              | "+fillChar('NICK', ' ', gameServer.config.playerMaxNickLength)+" | CELLS | SCORE  | POSITION     | DEAD"); // Fill space
        console.log(fillChar('', '-', ' ID         | IP              |  | CELLS | SCORE  | POSITION     | DEAD'.length + gameServer.config.playerMaxNickLength));
        for (var i = 0; i < gameServer.clients.length; i++) {
            var client = gameServer.clients[i].playerTracker;
            ...
            // Get name and data
            var nick = '', cells = '', score = '', position = '', deadTimes = '', data = '';
            if (client.spectate) {
                try { 
                ...
                data = fillChar("SPECTATING: " + nick, '-', ' | CELLS | SCORE  | POSITION     | DEAD'.length + gameServer.config.playerMaxNickLength, true);
